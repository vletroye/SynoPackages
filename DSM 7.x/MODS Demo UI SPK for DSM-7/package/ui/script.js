...
Ext.onReady(function() {
	...
	function onRunBtnClick(item){
		...
		saveBtn.disable();
		texta.setValue('Waiting for reply of demouispk7-cli '+cmdTxt.value+' (none could be ajax time-out)..');
		// when refresh is requested switch to bgmode; run with timeout: 150 instead default 30 mili-sec
		conn.request({
			//url: 'runcmd.cgi?'+Ext.urlEncode({cmd: 'demouispk7-cli'})+'&'+Ext.urlEncode({action: cmdTxt.value}),
			url: 'runcmd.cgi',
			params: Ext.urlEncode({cmd: 'demouispk7-cli', action: cmdTxt.value}),
			timeout: 150000,
			success: function(responseObject) {
